...
function MyProcurementsPage() {
  ...
  const fetchMyProcurements = async () => {
    ...
    try {
      ...
      console.error(
        'Помилка завантаження моїх закупівель:',
        err.response ? err.response.data : err.message
      );
      let errorMessage =
        'Не вдалося завантажити ваші закупівлі. Спробуйте пізніше.';
      if (err.response) {
  ...
  const handleDeleteProcurement = async (procurementId, procurementName) => {
    console.log(
      'Спроба видалення закупівлі. ID:',
      procurementId,
      'Тип ID:',
      typeof procurementId,
      'Назва:',
      procurementName
    );
    ...
    if (
      !window.confirm(
        `Ви впевнені, що хочете видалити закупівлю "${procurementName}"? Це скасує всі пов'язані пропозиції.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (err.response) {
        if (err.response.status === 401 || err.response.status === 403) {
          errorMessage =
            'У вас немає дозволу на видалення цієї закупівлі або ваша сесія закінчилася.';
  ...
  const filteredProcurements = useMemo(() => {
    ...
    return procurements.filter((p) => {
      ...
      const completionDateObj = p.completionDate
        ? new Date(p.completionDate)
        : null;
      const procurementCompletionDate = completionDateObj
        ? new Date(
            completionDateObj.getFullYear(),
            completionDateObj.getMonth(),
            completionDateObj.getDate()
          )
        : null;

      if (selectedStatusFilter === 'overdue') {
        return (
          procurementCompletionDate &&
          procurementCompletionDate < today &&
  ...
  const displayMessage =
    !loading && !error && filteredProcurements.length === 0 && !message
        ...
        {loading ? (
          ...
          <div className={classes.resultsContainer}>
            {filteredProcurements.map((procurement) => {
              ...
              return (
                <div
                  key={currentProcurementId || Math.random()}
                  className={classes.procurementCard}
                >
                  ...
                  <p>
                    ...
                    <span
                      style={{
                        ...
                        color:
                          procurement.status &&
                          procurement.status.toLowerCase() === 'open'
                            ? 'blue'
                            : procurement.status &&
                                procurement.status.toLowerCase() === 'fulfilled'
                              ? 'green'
                              : 'red',
                      }}
                    >
