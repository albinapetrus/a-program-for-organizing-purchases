...
export class CabinetCustComponent extends Component {
  ...
  fetchUserProfile = async () => {
    ...
    if (!jwtToken) {
      console.log(
        'CabinetCust: Токена немає при завантаженні профілю. Викликаємо logout і перенаправляємо.'
      );
      logout();
      this.navigate('/form');
      this.setState({
    ...
    try {
      ...
      let errorMessage =
        'Не вдалося завантажити профіль користувача. Спробуйте пізніше.';

      if (error.response) {
        if (error.response.status === 401) {
          errorMessage =
            'Сесія закінчилася або ви не авторизовані. Будь ласка, увійдіть знову.';
          console.log(
            'CabinetCust: Отримано 401 помилку. Викликаємо logout і перенаправляємо.'
          );
          logout();
          this.navigate('/form');
  ...
  handleLogout = () => {
    const { logout } = this.context;
    console.log(
      "CabinetCust: Кнопка 'Вийти' натиснута. Викликаємо logout і перенаправляємо."
    );
    logout();
    this.navigate('/form');
  };
  ...
  handleSaveCompanyName = async () => {
    ...
    if (newCompanyName.trim() === companyName.trim()) {
      this.setState({
        isEditingCompanyName: false,
        error: '',
        updateSuccess: 'Назва компанії не змінилася.',
      });
      return;
    }
    if (newCompanyName.trim() === '') {
      this.setState({
        error: 'Назва компанії не може бути порожньою.',
        updateSuccess: '',
      });
      return;
    }
    ...
    try {
      ...
      this.setState({
        ...
        loading: false,
        updateSuccess:
          response.data.message || 'Назва компанії успішно оновлена!',
        error: '',
      });
    } catch (error) {
      console.error(
        'Помилка оновлення назви компанії:',
        error.response ? error.response.data : error.message
      );
      let errorMessage =
        'Не вдалося оновити назву компанії. Спробуйте пізніше.';
      if (error.response) {
    ...
    return (
      <div className={classes.universal}>
                ...
                <p style={{ fontWeight: 'bold' }}>
                  ...
                  {isEditingCompanyName ? (
                    <div className={classes.companyNameEditGroup}>
                      ...
                      />
                      <button
                        onClick={this.handleSaveCompanyName}
        ...
        {role !== 'customer' && role !== 'supplier' && !loading && !error && (
          <div className={classes.help2}>
            <button className={classes.buttonHelp} onClick={this.handleLogout}>
              Вийти
            </button>
            <p style={{ marginTop: '10px', color: '#666' }}>
              Для вашої ролі додаткові дії на цій сторінці не передбачені.
            </p>
          </div>
        )}
      </div>
