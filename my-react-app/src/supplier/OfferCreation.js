...
function OfferCreationPage() {
  ...
  const [procurementDetailsLoading, setProcurementDetailsLoading] =
    useState(true);
  ...
  const fetchProcurementDetails = async () => {
    if (!procurementId) {
      setError(
        'ID закупівлі не знайдено. Будь ласка, перейдіть зі списку закупівель.'
      );
    ...
    try {
      ...
      setIsProcurementOpen(
        response.data.status && response.data.status.toLowerCase() === 'open'
      );
    } catch (err) {
      console.error(
      ...
      setProcurementDetailsError(
        `ID закупівлі: ${procurementId} (Помилка завантаження)`
      );
      setIsProcurementOpen(false);
    } finally {
      setProcurementDetailsLoading(false);
  ...
  const handleOpenSubmitConfirmModal = (e) => {
    ...
    if (!isProcurementOpen) {
      setError(
        `Ви не можете подати пропозицію на цю закупівлю, оскільки вона має статус "${procurementDetails ? translateStatus(procurementDetails.status) : 'Неактивна'}".`
      );
      return;
    }
    ...
    if (!supplierFullName) {
      setError(
        'Будь ласка, вкажіть повне найменування вашого ФОП або юридичної особи.'
      );
      return;
    }
    if (!supplierIban) {
  ...
  const handleSubmitOfferConfirmed = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        response.data.message || 'Пропозицію успішно надіслано!'
      );
      setProposedPrice('');
      setMessage('');
      ...
      if (err.response) {
        if (err.response.status === 401) {
          errorMessage =
            'Ви не авторизовані або ваша сесія закінчилася. Будь ласка, увійдіть знову.';
          ...
          errorMessage =
            'У вас немає дозволу на створення пропозицій. Це можуть робити лише постачальники.';
    ...
    <div className={classes.universal}>
      <div className={classes.block}>
        <h1 className={`${classes.label} ${classes.labelBlue}`}>
          Створити пропозицію для закупівлі: <br /> "
          {procurementDetails ? procurementDetails.name : 'Завантаження...'}"
        </h1>
      </div>
      <div className={classes.block}>
        {procurementDetailsLoading ? (
          ...
          <div className={classes.procurementDetailsCard}>
            ...
            <p>
              <strong>Назва:</strong> {procurementDetails.name}
            </p>
            <p>
              <strong>Категорія:</strong> {procurementDetails.category}
            </p>
            <p>
              <strong>Опис:</strong>{' '}
              {procurementDetails.description || 'Не вказано'}
            </p>
            <p>
              <strong>Кількість/Обсяг:</strong>{' '}
              {procurementDetails.quantityOrVolume}
            </p>
            <p>
              <strong>Орієнтовний бюджет:</strong> $
              {procurementDetails.estimatedBudget}
            </p>
            <p>
              <strong>Дата завершення:</strong>{' '}
              {new Date(procurementDetails.completionDate).toLocaleDateString()}
            </p>
            {procurementDetails.deliveryAddress && (
              <p>
                <strong>Адреса доставки замовника:</strong>{' '}
                {procurementDetails.deliveryAddress}
              </p>
            )}
            {procurementDetails.contactPhone && (
              <p>
                <strong>Телефон замовника:</strong>{' '}
                {procurementDetails.contactPhone}
              </p>
            )}
            {procurementDetails.documentPaths && (
              <p>
                ...
                <a
                  href={`${BACKEND_BASE_URL}${procurementDetails.documentPaths}`}
                  target="_blank"
            ...
            <p>
              ...
              {new Date(procurementDetails.createdAt).toLocaleDateString()}
            </p>
            <p>
              ...
              <span
                style={{
                  ...
                  color:
                    ...
                    procurementDetails.status.toLowerCase() === 'open'
                      ? 'green'
                      : procurementDetails.status &&
                          procurementDetails.status.toLowerCase() ===
                            'fulfilled'
                        ? 'blue'
                        : 'red',
                }}
        ...
        {!isProcurementOpen &&
          ...
          procurementDetails && (
            <p style={{ color: 'red', fontWeight: 'bold', marginTop: '1.5em' }}>
              Ця закупівля має статус "
              {translateStatus(procurementDetails.status)}", тому пропозиції на
              неї не приймаються.
            </p>
          )}
      </div>
      ...
      <div style={{ paddingLeft: '4em' }} className={classes.block}>
        <form
          onSubmit={handleOpenSubmitConfirmModal}
          className={classes.form}
          style={{ background: 'white' }}
        >
          <h2 style={{ background: 'white', marginBottom: '0.8em' }}>
            Заповніть форму пропозиції:
          </h2>
          ...
          <input
            ...
            disabled={loading || !isProcurementOpen}
          />
          <br />
          ...
          <label htmlFor="proposedDeliveryDate">
          ...
          <label htmlFor="supplierFullName">
            Повне найменування (ФОП/ТОВ):{' '}
          </label>
          <input
            type="text"
            id="supplierFullName"
            ...
            onChange={handleChange}
            placeholder="Наприклад, ФОП Іванов Іван Іванович"
            required
            className={classes.inputField}
            disabled={loading || !isProcurementOpen}
          />
          ...
          <input
            ...
            value={paymentEdrpou}
            onChange={handleChange}
            placeholder="Ваш ЄДРПОУ або РНОКПП"
            required
            className={classes.inputField}
            disabled={loading || !isProcurementOpen}
            style={{ width: '78.8%' }}
          ...
          <input
            ...
            value={paymentIpn}
            onChange={handleChange}
            placeholder="Ваш ІПН (для платників ПДВ)"
            className={classes.inputField}
            disabled={loading || !isProcurementOpen}
            style={{ width: '92%' }}
            required
          ...
          <label
            htmlFor="offerDocumentFile"
            className={classes.styledFile}
            style={{
              opacity: loading || !isProcurementOpen ? 0.6 : 1,
              cursor: loading || !isProcurementOpen ? 'not-allowed' : 'pointer',
          ...
          <button
            type="submit"
            style={{ width: '33em', marginLeft: '10em' }}
      ...
      {isSubmitConfirmModalOpen && (
        <div className={classes.modalOverlay} onClick={closeSubmitConfirmModal}>
          ...
          >
            ...
            <p>
              Підтверджуючи, ви зобов'язуєтесь доставити товар (або надати
              послугу) в зазначеному замовником обсязі "
              {procurementDetails?.name || 'обраної закупівлі'}" до{' '}
              <strong>
                {proposedDeliveryDate
                  ? new Date(proposedDeliveryDate).toLocaleDateString()
                  : 'вказаної дати'}
              </strong>{' '}
              включно, у належній якості, в разі якщо замовник прийме вашу
              пропозицію.
            </p>
            <div
              style={{
